...
void	main_menu_loop(t_window *window, void *data)
{
	...
	t_vector2 size = get_text_size(game->assets.main_font, "PACASTING");
	mlxe_draw_text(window, game->assets.main_font, "PACASTING", (t_vector2){window->width / 2 - size.x / 2, window->height / 6 - size.y / 2});
	mlxe_font_set_size(game->assets.main_font, 64);
	size = get_text_size(game->assets.main_font, "MOVE TO START");
	mlxe_draw_text(window, game->assets.main_font, "MOVE TO START", (t_vector2){window->width / 2 - size.x / 2, (window->height / 6.f)*4 - size.y / 2 + cos(i) * 10});
	i = fmod(i + 0.06, 2 * 3.14159f);
	if (mlxe_is_key_down(window, XK_w) || mlxe_is_key_down(window, XK_a) || mlxe_is_key_down(window, XK_s) || mlxe_is_key_down(window, XK_d))
	{
